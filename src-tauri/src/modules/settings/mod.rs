...
pub fn read_settings(app: &tauri::AppHandle) -> Result<AppSettings, String> {
    ...
    if let Ok(s) = serde_json::from_slice::<AppSettings>(&data) { return Ok(s); }
    if let Ok(legacy) = serde_json::from_slice::<SettingsFileLegacy>(&data) {
        return Ok(AppSettings { api_key: Some(legacy.api_key), api_url: legacy.api_url, ..Default::default() });
    }
...
pub fn write_settings(app: &tauri::AppHandle, settings: &AppSettings) -> Result<(), String> {
    let path = settings_path(app)?;
    if let Some(parent) = path.parent() { fs::create_dir_all(parent).map_err(|e| format!("create parent dir failed: {e}"))?; }
    let json = serde_json::to_vec_pretty(settings).map_err(|e| format!("serialize settings failed: {e}"))?;
    let mut file = fs::File::create(&path).map_err(|e| format!("open settings file failed: {e}"))?;
    file.write_all(&json).map_err(|e| format!("write settings file failed: {e}"))?;
    Ok(())
}

pub fn merge_settings(base: &mut AppSettings, update: AppSettings) {
    if update.api_key.is_some() { base.api_key = update.api_key; }
    if update.api_url.is_some() { base.api_url = update.api_url; }
    if update.data_dir.is_some() { base.data_dir = update.data_dir; }
    if update.spell_check.is_some() { base.spell_check = update.spell_check; }
    if update.experimental.is_some() { base.experimental = update.experimental; }
    if update.huggingface_token.is_some() { base.huggingface_token = update.huggingface_token; }
    if update.openai_api_key.is_some() { base.openai_api_key = update.openai_api_key; }
    if update.anthropic_api_key.is_some() { base.anthropic_api_key = update.anthropic_api_key; }
    ...
    if let Some(up) = update.appearance {
        ...
        if up.font_size.is_some() { current.font_size = up.font_size; }
        if up.code_show_line_numbers.is_some() { current.code_show_line_numbers = up.code_show_line_numbers; }
        if up.code_theme.is_some() { current.code_theme = up.code_theme; }
        if up.chat_width.is_some() { current.chat_width = up.chat_width; }
        if up.window_bg.is_some() { current.window_bg = up.window_bg; }
        if up.app_bg.is_some() { current.app_bg = up.app_bg; }
        if up.primary_color.is_some() { current.primary_color = up.primary_color; }
        if up.accent_color.is_some() { current.accent_color = up.accent_color; }
        if up.destructive_color.is_some() { current.destructive_color = up.destructive_color; }
    }
}
...
pub fn setup_provider_env_for_model(app: &tauri::AppHandle, model: &str) {
    ...
    if model.starts_with("gpt-") || model.contains("o1") || model.contains("gpt4") {
        if let Some(k) = settings.openai_api_key.or(settings.api_key.clone()) { std::env::set_var("OPENAI_API_KEY", k); }
        if let Some(url) = settings.openai_base_url { std::env::set_var("OPENAI_BASE_URL", url); }
    } else if model.starts_with("claude-") {
        if let Some(k) = settings.anthropic_api_key.clone() { std::env::set_var("ANTHROPIC_API_KEY", k); }
        if let Some(url) = settings.anthropic_base_url { std::env::set_var("ANTHROPIC_BASE_URL", url); }
    } else if model.starts_with("gemini-") || model.contains("gemini") {
        if let Some(k) = settings.gemini_api_key.clone() { std::env::set_var("GEMINI_API_KEY", k); }
        if let Some(url) = settings.gemini_base_url { std::env::set_var("GEMINI_BASE_URL", url); }
...
#[tauri::command]
pub async fn update_settings(app: tauri::AppHandle, update: AppSettings) -> Result<AppSettings, String> {
...
pub async fn reset_settings(app: tauri::AppHandle) -> Result<(), String> {
    let path = settings_path(&app)?;
    if path.exists() { fs::remove_file(&path).map_err(|e| format!("remove settings failed: {e}"))?; }
    Ok(())
}
...
pub async fn get_general_info(app: tauri::AppHandle) -> Result<GeneralInfo, String> {
    let app_data_dir = app.path().app_data_dir().map_err(|e| format!("paths: {e}"))?;
    let logs_dir = app.path().app_log_dir().map_err(|e| format!("paths: {e}"))?;
    Ok(GeneralInfo {
...
#[tauri::command]
pub async fn save_settings(app: tauri::AppHandle, api_key: String, api_url: Option<String>) -> Result<(), String> {
    let mut current = read_settings(&app)?;
...
pub async fn load_settings(app: tauri::AppHandle) -> Result<Option<SettingsFileLegacy>, String> {
    ...
    if let Some(api_key) = current.api_key.clone() {
        return Ok(Some(SettingsFileLegacy { api_key, api_url: current.api_url.clone() }));
    }
    Ok(None)
}
