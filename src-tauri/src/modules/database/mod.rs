...
#[tauri::command]
pub async fn db_archive_conversation(
    app: tauri::AppHandle,
    id: String,
    archived: bool,
) -> Result<(), String> {
...
#[tauri::command]
pub async fn db_update_conversation_title(
    app: tauri::AppHandle,
    id: String,
    title: String,
) -> Result<Conversation, String> {
    ...
    .map_err(|e| e.to_string())?;
    let mut stmt = conn
        .prepare("SELECT id, title, created_at, archived, model FROM conversations WHERE id = ?")
        .map_err(|e| e.to_string())?;
...
// db_generate_conversation_title has been removed - frontend handles title generation with AI SDK

#[tauri::command]
pub async fn db_update_conversation_model(
    app: tauri::AppHandle,
    conversation_id: String,
    model: String,
) -> Result<(), String> {
...
#[tauri::command]
pub async fn db_get_ai_messages(
    app: tauri::AppHandle,
    conversation_id: String,
    ...
    let mut count_stmt = conn
        ...
        .map_err(|e| e.to_string())?;
    let _count: i64 = count_stmt
        .query_row([conversation_id.clone()], |row| row.get(0))
        .map_err(|e| e.to_string())?;
    ...
    let rows = stmt
        .query_map([conversation_id.clone()], |row| {
...
#[tauri::command]
pub async fn db_add_ai_message(
    app: tauri::AppHandle,
...
#[tauri::command]
pub async fn db_save_complete_message(
    app: tauri::AppHandle,
    input: SaveCompleteMessageInput,
) -> Result<(), String> {
    let conn = get_conn(&app)?;
    ...
    if !message_with_meta["conversation_id"].is_string() {
        message_with_meta["conversation_id"] =
            serde_json::Value::String(input.conversation_id.clone());
    }
    if !message_with_meta["created_at"].is_string() {
        message_with_meta["created_at"] =
    ...
    let created_at = message_with_meta["created_at"]
        .as_str()
