...
pub async fn get_hardware_info(state: tauri::State<'_, AppSys>) -> Result<HardwareInfo, String> {
    ...
    if sys.cpus().is_empty() { *sys = System::new_all(); }
    sys.refresh_all();
    ...
    let cpu = CpuInfo { model: cpu_model, architecture: arch, physical_cores };

    // Memory
    let total_gb = (sys.total_memory() as f64) / (1024.0 * 1024.0 * 1024.0);
    let avail_gb = (sys.available_memory() as f64) / (1024.0 * 1024.0 * 1024.0);
    let used_percent = if total_gb > 0.0 { ((total_gb - avail_gb) / total_gb) * 100.0 } else { 0.0 };
    ...
    Ok(HardwareInfo { os_name, os_version, cpu, memory, gpus })
}
