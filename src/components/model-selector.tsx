	...
	useEffect(() => {
		(async () => {
			try {
				...
				try {
					const { initializeGoogle } = await import(
						"../components/providers/google"
					);
				...
				const allModels = [
					...
					...aiSdkModels.filter(
						(sdkModel) =>
							!backendModels.some(
								(backendModel) => backendModel.model === sdkModel.model,
							),
					),
				];
				...
				const modelToUse =
					storedModel && allModels.find((m) => m.model === storedModel)
						? storedModel
						: def;
				...
				>
					{selectedMeta ? (
						<>
							<ModelIcon
								className="size-4"
								model={toIcon(selectedMeta.adapter_kind)}
							/>
