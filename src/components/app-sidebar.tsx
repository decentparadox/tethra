...
export function AppSidebar() {
	...
	const handleConversationHover = (conversationId: string) => {
		if (
			!chatCache.hasData(conversationId) &&
			!chatCache.isPreloading(conversationId)
		) {
			// Start preloading after a short delay
			setTimeout(async () => {
				if (!chatCache.hasData(conversationId)) {
					await chatCache.preloadConversations(
						[conversationId],
						getLegacyMessages,
						1,
					);
								...
								<>
									<div className="flex items-center gap-1 text-left-panel-fg/80">
										<svg
											...
											fill="currentColor"
											stroke="none"
											className="tabler-icon tabler-icon-message-filled "
										>
									...
									<div className="py-2 text-white/60 text-left text-xs">
										Start a new conversation to see your conversation history
										here.
									</div>
								</>
							) : (
										...
										<div className="flex items-center justify-between gap-2">
											...
											<DropdownMenu>
												...
												<DropdownMenuContent align="end">
													<DropdownMenuItem
														onClick={async () => {
															await archiveConversation(c.id, c.archived === 0);
															await refresh();
														}}
													>
													...
													<DropdownMenuItem
														onClick={() => {
															navigator.clipboard.writeText(
																`${location.origin}/dashboard/${c.id}`,
															);
														}}
													>
