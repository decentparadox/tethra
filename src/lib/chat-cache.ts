import type { LegacyMessage, Conversation } from "./chat";
...
class ChatCache {
	...
	async preloadConversations(
		...
		const toPreload = conversationIds.filter(
			(id) => !this.hasData(id) && !this.preloadingSet.has(id),
		);
			...
			await Promise.allSettled(
				batch.map(async (conversationId) => {
					try {
						...
						console.log(
							`✅ Preloaded conversation: ${conversationId.slice(0, 8)}...`,
						);
					} catch (error) {
						console.log(
							`❌ Failed to preload conversation ${conversationId.slice(0, 8)}...`,
						);
					} finally {
	...
	getAdjacentConversationIds(currentId: string): {
		prev?: string;
		next?: string;
	} {
		const currentIndex = this.conversations.findIndex(
			(c) => c.id === currentId,
		);
