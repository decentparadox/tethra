...
export async function generateTitle(props: GenerateTitleProps) {
  ...
  try {
    ...
    const messageText = firstMessage.parts
...
export async function detectContextChange(props: DetectContextChangeProps): Promise<{ hasChanged: boolean; newTitle?: string }> {
  const { chatId, recentMessages, model, currentTitle, messagesSinceLastUpdate = 0 } = props;
...
- If context unchanged: NO_CHANGE`
      ...
      system: "You are analyzing conversation context changes. Be conservative - only suggest title changes when there's a clear, significant shift in the main topic of discussion.",
    });
    ...
    if (response.startsWith("NEW_TITLE:")) {
      ...
      if (cleanedTitle && cleanedTitle.length >= 3 && cleanedTitle !== currentTitle) {
        try {
          // Update the conversation title in the database
