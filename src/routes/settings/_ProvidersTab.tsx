...
export default function ProvidersTab() {
		...
		try {
			...
			if (active) {
				...
				if (!oldKeyValue && newKeyValue && newKeyValue.trim().length > 0) {
					const currentModels =
						(saved[meta.modelsField] as string[] | null) ?? [];
					if (currentModels.length === 0) {
						try {
							const fetchedModelsList = await invoke<string[]>(
								"get_adapter_models",
								{ adapterKind: active },
							);
	...
	const fetchAdapterModels = async (adapterKind: ProviderKey) => {
		...
		setFetchError(null);
		try {
			const models = await invoke<string[]>("get_adapter_models", {
				adapterKind,
			});
			...
			{active === null ? (
				<div className="w-full">
					<div className="flex items-center justify-between mb-2">
						...
						<button
							className="px-2 py-1 text-xs rounded-md bg-white/10 border border-white/10 inline-flex items-center gap-1"
							disabled
						>
							+ Add Provider
						</button>
					...
					<div className="divide-y divide-white/10 rounded-lg border border-white/10">
						{(
							...
							const enabledField =
								`${p.toLowerCase()}_enabled` as keyof Settings;
							const modelsField = providerMeta[p].modelsField;
							const modelCount = (
								(settings[modelsField] as string[] | null) ?? []
							).length;
							const isEnabled =
								(settings[enabledField] as boolean | null) ?? true;
							return (
								<div
									key={p}
									...
									<div className="flex items-center gap-2">
										...
										<label className="inline-flex items-center cursor-pointer">
											<input
												...
												checked={!!isEnabled}
												onChange={(e) =>
													update({ [enabledField]: e.target.checked } as any)
												}
											/>
					...
					<div className="rounded-lg border border-white/10 bg-white/5 p-4">
						...
						<div className="flex items-center gap-2">
							<input
								...
								value={
									(settings[providerMeta[active].keyField] as string) ?? ""
								}
								onChange={(e) =>
									update({
										[providerMeta[active].keyField]: e.target.value,
									} as any)
								}
							/>
							<button
						...
						<div className="mt-3">
							...
							<input
								...
								value={
									(settings[providerMeta[active].baseUrlField] as string) ?? ""
								}
								onChange={(e) =>
									update({
										[providerMeta[active].baseUrlField]: e.target.value,
									} as any)
								}
							/>
						</div>
					...
					<div className="rounded-lg border border-white/10 bg-white/5">
						<div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
							...
							<div className="flex items-center gap-2">
								...
								<button
									className="inline-flex items-center gap-1 px-2 py-1 text-xs rounded-md bg-white/10 border border-white/10"
									onClick={addModel}
								>
									<Plus size={14} /> Add
								</button>
									...
									<div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-40 overflow-y-auto">
										{fetchedModels
											...
											.map((model) => (
												<div
													key={model}
													className="text-xs bg-white/5 border border-white/10 rounded px-2 py-1 flex items-center justify-between"
												>
										...
										{fetchedModels.filter((model) => !models.includes(model))
											.length === 0 &&
											fetchedModels.length > 0 && (
							...
							{active === "OpenRouter" && showCustomInput && (
								<div className="px-4 py-3 border-t border-white/10 bg-white/5">
									...
									<div className="text-xs text-white/60 mb-3">
										Enter a model ID from OpenRouter (e.g.,
