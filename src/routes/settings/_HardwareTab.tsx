...
export default function HardwareTab() {
	...
	useEffect(() => {
		...
		const tick = async () => {
			try {
				const s = await invoke<UsageSnapshot>("get_usage_snapshot");
				setUsage(s);
			} catch {}
	...
	return (
		<div className="flex flex-col gap-4">
					...
					right={
						<>
							...
							<div className="text-right text-xs opacity-70 mt-1">
								{usage ? `${usage.cpu_percent.toFixed(2)}%` : "-"}
							</div>
						</>
					}
				/>
			...
			<SettingsSection title="Memory">
				<Row
					...
					right={
						<>
							<div>
								{(usage?.total_gb ?? hw?.memory.total_gb ?? 0).toFixed(2)} GB
							</div>
						</>
					}
				/>
				<Row
					...
					right={
						<>
							<div>
								{(usage?.available_gb ?? hw?.memory.available_gb ?? 0).toFixed(
									2,
								)}{" "}
								GB
							</div>
				...
				<Row
					...
					right={
						<>
							<ProgressBar
								percent={
									usage?.mem_used_percent ?? hw?.memory.used_percent ?? 0
								}
							/>
							<div className="text-right text-xs opacity-70 mt-1">
								{(
									usage?.mem_used_percent ??
									hw?.memory.used_percent ??
									0
								).toFixed(2)}
								%
							</div>
			...
			<SettingsSection title="GPUs">
				{(hw?.gpus?.length ? hw.gpus : [{ name: "Unknown GPU" }]).map(
					(g, i) => (
						<Row
							key={`${g.name}-${i}`}
							left={
